...
@section('title', 'Edit Calendar Mapping')

@section('subtitle', 'Update doctor-calendar association')

@section('content')
    <form method="POST" action="{{ route('admin.calendars.update', $calendar) }}" class="max-w-2xl">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm">
            <div class="p-6 space-y-6">
                <div>
                    <label for="doctor_id" class="block text-sm font-medium text-[#0F1B3D] mb-1">Doctor</label>
                    <select
                        name="doctor_id"
                        id="doctor_id"
                    ...
                    >
                        ...
                        @foreach ($doctors as $doctor)
                            <option value="{{ $doctor->id }}" {{ old('doctor_id', $calendar->doctor_id) == $doctor->id ? 'selected' : '' }}>
                                {{ $doctor->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
                        <label for="plato_calendar_color_id" class="block text-sm font-medium text-[#0F1B3D] mb-1">Plato Calendar Color ID</label>
                        <input
                            type="text"
                            name="plato_calendar_color_id"
                            id="plato_calendar_color_id"
                            value="{{ old('plato_calendar_color_id', $calendar->plato_calendar_color_id) }}"
                ...
                <div>
                    <label class="inline-flex items-center gap-2 cursor-pointer">
                        ...
                        <input
                            ...
                            id="is_active"
                            value="1"
                            {{ old('is_active', $calendar->is_active) ? 'checked' : '' }}
