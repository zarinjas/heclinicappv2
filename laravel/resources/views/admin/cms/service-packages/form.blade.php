...
@section('subtitle', $isEdit ? 'Update package image, name, description, items, or status' : 'Upload a new service package for the mobile app')
    ...
    <form method="POST"
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm">
            <div class="p-6 space-y-6">
                ...
                <div>
                    <label for="items" class="block text-sm font-medium text-[#0F1B3D] mb-1">Package Items</label>
                    <textarea
                        name="items"
                        id="items"
                        rows="6"
                        class="w-full px-4 py-2 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#00C9A7] focus:border-transparent outline-none @error('items') border-red-300 @enderror"
                        placeholder="EM Kegel x 1 session&#10;Pelvic Physiotherapy x 3 sessions&#10;Doctor Consultation"
                    >{{ old('items', $package->exists && $package->items ? implode(PHP_EOL, $package->items) : '') }}</textarea>
                    <p class="mt-1 text-xs text-gray-400">One item per line. Shown as a list in the mobile app. Prices are intentionally not shown — customers enquire via WhatsApp.</p>
                    @error('items')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
