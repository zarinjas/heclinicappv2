...
@section('title', 'CMS — He Clinic Info')

@section('subtitle', 'Manage clinic info images, order, and active status')
...
@section('content')
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div class="flex gap-2">
            ...
            <a href="{{ route('admin.cms.clinic-info.index', ['status' => '1']) }}"
            ...
            <a href="{{ route('admin.cms.clinic-info.index', ['status' => '0']) }}"
        ...
        <a href="{{ route('admin.cms.clinic-info.create') }}"
    ...
    @if ($items->isEmpty())
        <div class="bg-white rounded-xl border border-gray-100 p-12 text-center shadow-sm">
            ...
            <p class="text-sm text-gray-500 mb-4">No clinic info images found.</p>
            <a href="{{ route('admin.cms.clinic-info.create') }}"
    ...
    @else
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="bg-gray-50 border-b border-gray-100">
                            <th class="text-left px-6 py-3 font-medium text-gray-500">Preview</th>
                            <th class="text-left px-6 py-3 font-medium text-gray-500">Added</th>
                    ...
                    <tbody>
                        @foreach ($items as $item)
                            <tr class="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                                <td class="px-6 py-4">
                                    @if ($item->image_url)
                                        <img src="{{ $item->image_url }}" alt="" class="w-24 h-16 object-cover rounded border border-gray-200">
                                    @else
                                ...
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium {{ $item->is_active ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-500' }}">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('admin.cms.clinic-info.edit', $item) }}"
                                        ...
                                        <form method="POST" action="{{ route('admin.cms.clinic-info.destroy', $item) }}"
        ...
        <p class="text-xs text-gray-400 mt-4">
            Showing {{ $items->firstItem() ?: 0 }}–{{ $items->lastItem() ?: 0 }} of {{ $items->total() }} images
        </p>
    @endif
@endsection
