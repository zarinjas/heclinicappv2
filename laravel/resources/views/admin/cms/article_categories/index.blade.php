...
@section('title', 'CMS — Article Categories')

@section('subtitle', 'Manage article categories used across the mobile app')

@section('content')
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <a href="{{ route('admin.cms.articles.index') }}"
               class="text-sm text-gray-500 hover:text-[#0F1B3D] transition-colors">
                &larr; Back to Articles
            </a>
        </div>

        <a href="{{ route('admin.cms.article-categories.create') }}"
                            ...
                            <tr class="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('admin.cms.article-categories.edit', $category) }}"
                                        ...
                                        <form method="POST" action="{{ route('admin.cms.article-categories.destroy', $category) }}"
                                              onsubmit="return confirm('Delete this category? Articles in it will be uncategorized.');"
                                              class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
