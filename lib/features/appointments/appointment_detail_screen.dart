...
class _AppointmentDetailScreenState extends State<AppointmentDetailScreen> {
  ...
  Future<void> _loadDetail() async {
    if (!mounted || widget.appointmentId == null) {
    ...
    try {
      ...
      if (response.succeeded) {
        ...
        if (body is List && body.isNotEmpty) {
          ...
          if (mounted) {
            setState(() {
              _doctorName = _extractString(data, 'doctorname') ?? _doctorName;
              _specialty = _extractString(data, 'specialty') ?? '';
              _branch = _extractString(data, 'branch_name') ?? _branch;
              _date = starts.isNotEmpty ? starts.first : _date;
              ...
              _status = _extractString(data, 'status') ?? _status;
              _isLoading = false;
            });
          }
          ...
          if (mounted) {
            setState(() {
              _doctorName = _extractString(body, 'doctorname') ?? _doctorName;
              _specialty = _extractString(body, 'specialty') ?? '';
              _branch = _extractString(body, 'branch_name') ?? _branch;
              _appointmentType =
                  _extractString(body, 'appointment_type') ?? '';
              _notes = _extractString(body, 'notes') ?? '';
              _status = _extractString(body, 'status') ?? _status;
              _isLoading = false;
            });
          }
        } else {
