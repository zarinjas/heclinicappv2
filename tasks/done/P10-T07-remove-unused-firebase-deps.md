...
### What Was Done
Removed 10 unused npm packages from `firebase/functions/package.json` dependencies: braintree, stripe, razorpay (payment SDKs), @langchain/core, @langchain/langgraph, @langchain/openai, @langchain/google-genai, @langchain/anthropic (AI/LLM), @mux/mux-node (video), @onesignal/node-onesignal (push). Verified index.js has zero imports for any of these packages. Ran `npm install` to regenerate package-lock.json. Verified `require('./index.js')` succeeds without errors.

### Files Changed
- `firebase/functions/package.json` — MODIFIED: removed 10 unused dependencies
- `firebase/functions/package-lock.json` — REGENERATED: updated by `npm install`

### Decisions Made During Implementation
- Confirmed index.js uses only `firebase-functions` and `firebase-admin` imports — none of the 10 removed packages are referenced
- Kept `axios` and `qs` per task spec despite not being imported in index.js (may be used by api_manager.js or planned for future)

### Known Limitations
- Node version warning (v22 vs required v20) — non-blocking for cleanup task
- npm audit reports 17 vulnerabilities in remaining dependencies (eslint@6.8.0 deprecated) — out of scope for this task
